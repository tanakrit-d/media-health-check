...
class VideoValidator:
    ...
    def _needs_scan(self, filepath: Path, force: bool) -> bool:
        ...
        cursor = self.conn.execute(
            "SELECT mtime, size, is_corrupted, scan_type, last_scanned FROM scanned_files WHERE filepath = ?",
            (str(filepath),),
        )
        row = cursor.fetchone()
        ...
        db_mtime = row["mtime"]
        db_size = row["size"]
        was_corrupted = row["is_corrupted"]
    ...
    def validate_file(
        ...
        try:
            ...
            result = subprocess.run(
                cmd, capture_output=True, timeout=300 if deep_scan else 30
            )
    ...
    def scan(
        ...
        self._log(
            f"Skipping {skipped_count} up-to-date files. Scanning {len(files_to_scan)} files...",
            "info",
        )
            ...
            try:
                for i, future in enumerate(as_completed(future_to_file)):
                    ...
                    if not is_valid:
                        ...
                        if error_msg:
                            scan_errors.append(
                                {"filepath": str(filepath), "error": error_msg}
                            )
                    elif self.verbose:
    ...
    def _prune_db(self):
        ...
        to_delete = [
            row["filepath"] for row in cursor if not Path(row["filepath"]).exists()
        ]

        if to_delete:
            self.conn.executemany(
                "DELETE FROM scanned_files WHERE filepath = ?",
...
def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Ignore history and rescan everything"
    )
    parser.add_argument(
        "--deep",
        action="store_true",
        help="Use ffmpeg decoding (slow) instead of ffprobe",
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    parser.add_argument(
        "--prune", action="store_true", help="Remove DB entries for deleted files"
    )
